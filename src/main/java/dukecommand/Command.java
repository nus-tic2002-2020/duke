...
public class Command {
    ...
    public void check(TaskList tasks) throws DukeException {
        if (!validCommand.contains(command[0])) {
            ...
            throw new DukeException("   Todo task does not accept /by schedule, please create Deadline task");
        } else if (command[0].equals("task") && !command[2].contains("/at")) {
            throw new DukeException("   Todo task does not accept /at schedule, please create Event task");
    ...
    public void execute(TaskList tasks, Ui ui, Storage storage) {
        switch (command[0]) {
            ...
            case "todo":
                ...
                storage.writeTask(tasks.getTask(tasks.size() - 1));
                break;
            case "deadline":
